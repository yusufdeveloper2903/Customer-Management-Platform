<template>
  <div class="additional-classes">
    <div class="avatar-tooltip cursor-pointer w-10 h-10 block relative hover:shadow-lg ml-3.5">
      <div v-if="!userImage" class="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-warning">
        <Icon icon="User" color="#356c2d" />
      </div>

      <div v-else class="mx-auto flex h-10 w-10 items-center justify-center rounded-full overflow-hidden">
        <img class="w-full h-full object-cover" :src="userImage" alt="Rounded avatar" />
      </div>
      <span class="-bottom-0.5 left-7 absolute w-2.5 h-2.5 bg-green-400 border border-primary rounded-full"></span>
    </div>
    <div v-show="sidebar.miniSidebar" uk-dropdown="pos: bottom-right;" class="bg-white dark:text-gray-300 p-0 py-2">
      <div class="flex justify-between items-center p-3 hover:bg-primary/10 hover:text-primary" @click="logout">
        <span class="text-success">{{ $t("logout") }}</span>
        <Icon icon="Logout 2" color="#356c2d" />
      </div>
    </div>
    ...
    <div v-if="sidebar.miniSidebar" class="horizontal-state">
      ...
      <div class="ml-6 mt-10 action-icons flex items-center">
        <span class="my-5 block cursor-pointer burger-btn" @click="sidebar.toggleSidebar()" uk-icon="menu" />
        ...
        <Icon class="block cursor-pointer close-on-phone" icon="Align Horizontal Center" color="#fff"
          @click="changeSidebarPosition" />
      </div>
      <div class="flex items-center gap-x-5">
        <Locale />
        <span class="bg-white/10 text-white rounded-full px-4 py-1 font-semibold text-sm">
    ...
    <div v-else class="vertical">
      <div v-if="check()" class="bg-white/10 mr-16 mt-2 h-7 text-white rounded-full px-4 py-1 font-semibold text-sm">
        <!-- {{ getUserData().role[0].name[locale] }} -->
      </div>
      ...
      <div class="ml-6 mt-10 action-icons">
        ...
        <Icon icon="Align Vertical Center" color="#fff" class="cursor-pointer close-on-phone"
          @click="changeSidebarPosition" />
      </div>
    </div>
  ...
  <div id="modal-search" class="uk-flex-top" uk-modal>
    <div class="uk-modal-dialog overflow-hidden rounded-md">
      <div class="uk-modal-header z-50 border-0 bg-white shadow">
        <form class="uk-search uk-search-default mb-8 w-full" @submit.prevent="">
          <span uk-search-icon style="margin-top: 4px" />
          <input id="search" v-model="search" type="search" autofocus :placeholder="$t('Search')"
            ...
            @keyup.enter="onEnter($event)" @input="onSearch($event.target.value)" />
        </form>
      </div>
      ...
      <div class="uk-modal-body h-[60vh] overflow-y-scroll">
        <div>
          ...
          <div v-for="(item, index) in items2" :key="index"
            ...
            uk-toggle="target: #modal-search" :class="index == 0 && items.length == 0 && search
