...
const props = defineProps<PropsBanner>();
const emits = defineEmits(["deleteBanner"]);
const deleteBanner = async () => {
  try {
    await store.deleteBanner(props.bannerId).then(() => {
      emits("deleteBanner");
...
<template>
  <div id="banner-delete-modal" class="uk-flex-top" uk-modal>
    <div
    ...
    >
      ...
      >
        <button uk-toggle="target: #banner-delete-modal" class="btn-secondary">
          {{ $t("Cancel") }}
        </button>
        <button
          uk-toggle="target: #banner-delete-modal"
