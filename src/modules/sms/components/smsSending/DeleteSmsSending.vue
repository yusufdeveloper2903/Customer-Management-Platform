...
const props = defineProps<PropsSms>();
const emits = defineEmits(["deleteSms"]);
const deleteSms = async () => {
  try {
    await store.deleteSmsSending(props.smsId).then(() => {
      emits("deleteSms");
...
<template>
  <div id="sms_sending-delete-modal" class="uk-flex-top" uk-modal>
    <div
    ...
    >
      ...
      >
        <button uk-toggle="target: #sms_sending-delete-modal" class="btn-secondary">
          {{ $t("Cancel") }}
        </button>
        <button
          uk-toggle="target: #sms_sending-delete-modal"
