...
import {phoneNumberFields, linkItems, linksFields, phoneNumberItems} from "../constants";
...
const items = ref<ICategory[]>(linkItems);
...
<!-- PHONE NUMBER TABLE -->
    ...
    <table class="min-w-full bg-white border border-gray-300 dark:border-gray-600">
      ...
      <tbody>
      <tr
          v-for="item in phoneNumberItems.sort(sortItems)"
          :key="item.index"
          class="border-y dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-darkLayoutMain dark:text-gray-200 cursor-move"
          :draggable="true"
          @dragstart="dragStart(item)"
      ...
      >
        ...
        <td class="px-6 whitespace-no-wrap">{{ item.phone }}</td>
        <td class="px-6 whitespace-no-wrap">
          <div class="flex py-2">
            <button
                class="btn-warning btn-action"
