...
const emits = defineEmits(["deleteSmsTemplate"]);
const deleteSmsTemplate = async () => {
  try {
    await store.deleteSmsTemplate(props.userId).then(() => {
      emits("deleteSmsTemplate");
...
<template>
  <div id="sms_template-delete-modal" class="uk-flex-top" uk-modal>
    <div
    ...
    >
      ...
      >
        <button uk-toggle="target: #sms_template-delete-modal" class="btn-secondary">
          {{ $t("Cancel") }}
        </button>
        <button
          uk-toggle="target: #sms_template-delete-modal"
          class="rounded-md btn-danger px-6 py-2 text-white duration-100 hover:opacity-90"
          @click="deleteSmsTemplate"
        >
          {{ $t("delete") }}
        </button>
      </div>
