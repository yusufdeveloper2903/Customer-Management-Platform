...
const validate: Ref<Validation> = useVuelidate(rules, cardData);

const store = users();
const registerStore = registration();
...
const emits = defineEmits(["createCard"]);
...
const resendSMSCode = async () => {
  try {
    cardData.value.number = removeSpaces(cardData.value.number);
    await registerStore.createUserCard(cardData.value);
    toast.success(t("sms_code_successfully_sent"));
...
const nextStep = async () => {
  ...
  if (!success) return;
  cardData.value.number = removeSpaces(cardData.value.number);
  store.createUserCard(cardData.value).then(() => {
...
watch(
    ...
    () => {
      if (props.exactCard) {
        cardData.value.id = props.exactCard.id;
        cardData.value.title = props.exactCard.title;
        cardData.value.number = props.exactCard.number;
        cardData.value.exp_date = props.exactCard.exp_date;
      }
    }
);
...
const saveCardDatas = () => {
  registerStore
      .confirmCreateUserCard({
        exp_date: cardData.value.exp_date,
        sms_code: sms_code.value,
        number: removeSpaces(cardData.value.number),
      })
      .then(() => {
        emits("createCard");
        UIkit.modal("#profile-actions-modal").hide();
        clearModal();
...
<template>
  <div id="profile-actions-modal" class="uk-flex-top" uk-modal>
    <div
    ...
    >
      ...
      <div class="uk-modal-body">
        <div v-show="!getSmsCode">
          <label for="cardNumber">{{ $t("map_name") }}</label>
          <input
              id="cardNumber"
          ...
          <div class="flex items-start gap-3 mb-5">
            <label for="cardNumber" class="w-2/3"
            >{{ $t("card_number") }}
              <input
                  id="cardNumber"
                  ...
                  :class="
                  validate.number.$errors.length ? 'required-input' : ''
                "
              />
              <p
            ...
            <label for="cardNumber" class="w-1/3"
            >{{ $t("term_card") }}
              <input
                  id="cardNumber"
                  placeholder="00 / 00"
                  ...
                  v-maska
                  data-maska="##/##"
                  v-model="cardData.exp_date"
                  :class="
                  validate.exp_date.$errors.length ? 'required-input' : ''
                "
              />
              <p
                  v-for="error in validate.exp_date.$errors"
                  :key="error.$uid"
                  class="text-danger text-sm"
              >
                {{ $t(error.$message) }}
